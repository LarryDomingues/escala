...
export default function Dashboard() {
  ...
  const isEventoEspecial = (escala) => {
    ...
    // Verificar se todos os campos estão vazios ou se tem algum marcador especial
  ...
  const getInstrumentoUsuario = (escala) => {
    ...
    for (const [campo, label] of Object.entries(instrumentos)) {
              ...
              {escalasExibir.map((escala, index) => {
                ...
                return (
                  <div 
                    key={index} 
                    className={`
                      p-3 md:p-4 hover:bg-gray-50 transition
                      ${estaEscalado ? 'bg-yellow-50 border-l-4 border-orange-400' : ''}
                      ${eventoEspecial ? 'bg-purple-50 border-l-4 border-purple-400' : ''}
                      ...
                      <div className="flex-1 min-w-[120px]">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className="text-xs text-gray-400">
                            {isToday(parseISO(escala.data + 'T00:00:00')) && '🔴 Hoje'}
                          </span>
                          {estaEscalado && (
                      ...
                      <div className="flex-1 min-w-[100px]">
                        {estaEscalado && instrumento ? (
                          <div className="flex items-center gap-1 text-sm font-medium text-orange-700">
                            <span className="text-lg">{instrumento.split(' ')[0]}</span>
                            <span>{instrumento.replace(/^[^\s]+\s/, '')}</span>
                          </div>
                        ) : eventoEspecial ? (
                          ...
                          <div className="text-xs text-gray-400">
                            {escala.voz_nome || escala.violao_nome || '--'}
                          </div>
                        )}
                      </div>
